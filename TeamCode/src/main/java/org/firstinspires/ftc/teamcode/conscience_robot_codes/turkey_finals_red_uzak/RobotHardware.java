package org.firstinspires.ftc.teamcode.conscience_robot_codes.turkey_finals_red_uzak;
...
public class RobotHardware {
    ...
    public int bankVelocity = 1800;
    public final int farVelocity = 1700;
    ...
    public final int TARGET_TAG_ID = 24; // for blue goal (change to 24 for red)


    // PID-Controller Gains (Tune these to fit your robot)
    private final double TURN_GAIN = 0.3; // P-Gain (Kp)
    private final double DERIVATIVE_GAIN = 0.05;    // D-Gain (Kd)
    private final double INTEGRAL_GAIN = 0.01; // I-Gain (Ki)
    private final double MAX_AUTO_TURN = 1.0;
    ...
    public void init(LinearOpMode opMode, HardwareMap hwMap) {
        ...
        double newP = pidfOrig.p;
        double newF = pidfOrig.f;
        double newI = pidfOrig.i;
        double newD = pidfOrig.d;
        PIDFCoefficients pidfNew = new PIDFCoefficients(newP, newI, newD, newF);
    ...
    public boolean updateAngularAlignment(AprilTagDetection target) {
        ...
        double headingError = target.ftcPose.bearing - 1;
    ...
    public void setShooterVelocity(double velocity) {
        ...
        for (com.qualcomm.robotcore.hardware.VoltageSensor sensor : myOpMode.hardwareMap.voltageSensor) {
            batteryVoltage = sensor.getVoltage();
        }
        double referenceVoltage = 13.7;
        double compensationFactor = 1.0;
        if (batteryVoltage < referenceVoltage) {
            compensationFactor = referenceVoltage / batteryVoltage;
        }
        compensationFactor = Math.min(compensationFactor, 1.3);
        flywheel.setVelocity(velocity * compensationFactor);
    }
    ...
    public void bankShotAuto() {
        ...
        if (flywheel.getVelocity() >= bankVelocity - -5) {
            coreHex.setPower(1);
            myOpMode.sleep(250);
            coreHex.setPower(0);
        } else {
    ...
    public void adjustBankVelocity(int increment) {
        bankVelocity += increment;
        // Güvenlik limitleri (Örneğin 0 ile 2500 arası)
        if (bankVelocity < 0) bankVelocity = 0;
        if (bankVelocity > 2500) bankVelocity = 2500;
    }
